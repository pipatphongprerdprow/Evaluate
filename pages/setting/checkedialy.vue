        ...
        <div class="col md:col-5 text-right">
          ...
          >
            ...
            <DataTable :value="personPlansSortedByNumber" v-model:expandedRows="expandedPlansPerson" dataKey="planLabel" responsiveLayout="scroll" stripedRows>  
              <Column header="ไตรมาส" style="width: 9.5rem; min-width: 9rem; text-align:center;">
                  <template #body="slotProps">
                    <Tag :value="getQuarter(slotProps.data.startDate)" severity="info" />
                  </template>
              </Column> 
              <Column header="ปี" style="width: 6rem; min-width: 5.5rem; text-align:center;">
                <template #body="slotProps">
...
function getQuarter(dateInput) {
  ...
  if (m >= 10 && m <= 12) return 'ไตรมาส 1 : ต.ค. - ธ.ค.';
  if (m >= 1 && m <= 3)  return 'ไตรมาส 2 : ม.ค. - มี.ค.';
