            ...
            <Column field="Tb_score" header="หลักฐานที่แสดงถึงผลการปฎิบัติกราชการตามเกณฑ์การประเมิน (หลักฐานเชิงประจักษ์)" style="width: 20%">
              <template #body="Item"> 
                ...
                <p v-for="(xx, index) in Item.data.sub_ITemDoc" :key="index" class="mb-0">  
                    <a :href="'http://survey.msu.ac.th/evaluatebackend/storage/uploads/'+xx.doc_file" target="_blank">{{ xx.doc_name }}</a> <br>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      addscorep03(item) {
        console.log(item); 
        axios.post('/evaluatebackend/api/selectDatascorep', {
          p01_id: item,
        }).then((res) => {
          console.log(res.data); 
