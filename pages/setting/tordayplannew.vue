    ...
    <div class="card p-6 shadow-2xl rounded-xl surface-card">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-12 gap-3 mb-4 items-center">
        <div class="sm:col-6 flex justify-start">
          <Button icon="pi pi-plus" label="สร้างแผนงาน/โครงการใหม่"
                  class="p-button-info px-4 py-2 rounded-lg font-semibold shadow h-[42px]"
        ...
        <Column header="ผู้รับผิดชอบ" style="min-width:12rem;max-width:18rem">
          <template #body="{data}">
            <div class="flex flex-wrap gap-1">
              <span v-for="o in (data.owner || [])" :key="o.id" class="owner-chip">
                {{ getOwnerDisplay(o) }}
              </span>
        ...
        <Column header="จัดการ" style="width:8.5rem" class="text-center">
          <template #body="{ data }">
            <div class="flex gap-1 justify-center">
              <Button icon="pi pi-plus" class="p-button-text p-button-success p-button-sm p-button-rounded"
                      @click="openAddStepDialogForPlan(data)" v-tooltip="'เพิ่มขั้นตอน/กิจกรรม'"/>
              <Button icon="pi pi-pencil" class="p-button-text p-button-warning p-button-sm p-button-rounded"
                      @click="editPlan(data)" v-tooltip="'แก้ไขโครงการ'"/>
              <Button icon="pi pi-trash" class="p-button-text p-button-danger p-button-sm p-button-rounded"
                      @click="confirmDeletePlan(data.id)" v-tooltip="'ลบโครงการ'"/>
            </div>
          </template>
        </Column>
              ...
              <Column header="จัดการ" style="width:12rem" class="text-center">
                <template #body="{data}">
                  <div class="flex gap-2 justify-center">
                    <Button icon="pi pi-plus" label="เพิ่ม" class="p-button-warning p-button-sm px-3 py-1"
                            @click="openAddTaskDialog(data, planSlot.data.owner)"/>
                    <Button icon="pi pi-pencil" class="p-button-text p-button-warning p-button-sm p-button-rounded"
                            @click="openEditStepDialogInTable(planSlot.data.id, data)"/>
                    <Button icon="pi pi-trash" class="p-button-text p-button-danger p-button-sm p-button-rounded"
                            @click="confirmRemoveStepById(planSlot.data.id, data.id)"/>
                  </div>
                </template>
              </Column>
              ...
              <template #expansion="stepProps">
                <div class="p-4 bg-gray-100 border-round-xl ml-4">
                  ...
                  <DataTable :value="stepProps.data.tasks" responsiveLayout="scroll" stripedRows :class="{'p-datatable-gridlines': true}">
                    ...
                    <Column header="แบบ ป. ที่สอดคล้อง" style="min-width:12rem">
                      <template #body="{data}">
                    ...
                    <Column header="เวลาที่ใช้ไป" style="width:7rem" class="text-center">
                      <template #body="{data}">{{ getTaskTimeSpent(data) }}</template>
                    </Column>
                    ...
                    <Column header="จัดการ" style="width:6rem" class="text-center">
                      <template #body="{data, index}">
                        <div class="flex gap-2 justify-center">
                          <Button icon="pi pi-pencil" class="p-button-text p-button-warning p-button-sm p-button-rounded"
                                  @click="openEditTaskDialogInTable(stepProps.data, data, index)"/>
                          <Button icon="pi pi-trash" class="p-button-text p-button-danger p-button-sm p-button-rounded"
                                  @click="confirmRemoveTaskInTable(stepProps.data, index)"/>
                        </div>
                      </template>
                    </Column>
                  </DataTable>
    ...
    <Dialog v-model:visible="showPlanDialog" :breakpoints="{ '960px': '90vw' }" :style="{ width: '80vw' }" modal position="top">
      ...
      <div v-if="isEditMode || activeTabIndex===0">
        <h3 class="text-xl font-bold text-700 flex items-center mb-4">
          <i class="pi pi-file mr-2 text-primary-500"></i>1. ข้อมูลแผนงาน / โครงการ
        </h3>

        <div class="p-fluid grid">
          <div class="field col-3">
            <label class="font-semibold text-lg">ปีงบประมาณ <span class="text-red-500">*</span></label>
            <Dropdown v-model="currentPlan.planYear" :options="planYears" optionLabel="label" optionValue="value"
                      placeholder="เลือกปีงบประมาณ" class="w-full" required/>
          </div>

          <div class="field col-9">
            <label class="font-semibold text-lg">ประเภทแผน <span class="text-red-500">*</span></label>
            <Dropdown v-model="currentPlan.planType" :options="planTypes" optionLabel="label" optionValue="value"
                      placeholder="เลือกประเภทแผน" class="w-full" required/>
          </div>
      ...
      <div v-else-if="!isEditMode && activeTabIndex===1">
        <h3 class="text-xl font-bold text-700 flex items-center mb-4">
          <i class="pi pi-list mr-2 text-primary-500"></i>2. กำหนดขั้นตอน/กิจกรรมการทำงาน
        </h3>
    ...
    <Dialog v-model:visible="showAddStepForPlanDialog" :breakpoints="{ '960px': '90vw' }" :style="{ width: '40vw' }" modal>
      ...
      <div class="p-fluid">
        ...
        <div class="field">
          ...
          <Calendar v-model="addStepForm.dates" selectionMode="range" :manualInput="false" placeholder="เลือกวันเริ่มต้น-สิ้นสุด"/>
    ...
    <Dialog v-model:visible="showEditTaskDialog" :breakpoints="{ '960px': '90vw' }" :style="{ width: '70vw' }" modal>
      ...
      <div class="p-fluid formgrid grid">
        <div class="field col-12 md:col-6">
          ...
          <InputText v-model="currentEditingTask.description" placeholder="ระบุภาระงาน..." class="w-full"/>
        </div>
        <div class="field col-12 md:col-3">
          <label class="font-semibold">เวลาเริ่มต้น</label>
          <Calendar v-model="currentEditingTask.startTime" :showTime="true" hourFormat="24" placeholder="เวลาเริ่มต้น" class="w-full"/>
        </div>
        <div class="field col-12 md:col-3">
          <label class="font-semibold">เวลาสิ้นสุด</label>
          <Calendar v-model="currentEditingTask.endTime" :showTime="true" hourFormat="24" placeholder="เวลาสิ้นสุด" class="w-full"/>
        </div>
      </div>
...
function normalizeStartEndSameDay(start, end, baseDate) {
  const base = baseDate || start || end || new Date()
  const s = start ? setTimeOnDate(base, start) : null
  const e = end ? setTimeOnDate(base, end) : null
  return { s, e }
}

function isTimeRangeValid(start, end, baseDate) {
...
const isFirstStepValid = computed(() =>
  ...
  !!(currentPlan.planLabel || '').trim() &&
  (currentPlan.owner?.length || 0) > 0 &&
...
const isNewTaskInStepValid = computed(() => {
  ...
  const timeOk = isTimeRangeValid(t.startTime, t.endTime, base)

  const needDesc = t.taskType !== 'งานอื่นๆ'
  if (needDesc) {
    return !!(t.description || '').trim() && !!t.status && timeOk
  }
  return !!t.status && timeOk
})
...
const formatDate = (x) => {
  ...
  return `${pad(d.getDate())}/${pad(d.getMonth() + 1)}/${yearBE}`
}
...
function mapApiToState(arr) {
  return (arr || []).map(p => {
    ...
    const planYearNorm = (p.planYear ?? p.plan_year ?? getBudgetYear(rawStart) ?? null)
...
async function savePlan() {
  ...
  if (!valid) {
    return Swal.fire({
      icon: 'error',
      title: 'ข้อผิดพลาด',
      text: isEditMode.value ? 'กรุณากรอกข้อมูลแผนงานให้ครบถ้วน' : 'กรุณาตรวจสอบข้อมูลแผนงานและขั้นตอน'
    })
  }

  const planYear = currentPlan.planYear?.value ?? currentPlan.planYear?.id ?? currentPlan.planYear
  const planType = currentPlan.planType?.value ?? currentPlan.planType?.id ?? currentPlan.planType

  if (!planYear) return Swal.fire({ icon: 'error', title: 'ข้อผิดพลาด', text: 'กรุณาเลือกปีงบประมาณ' })
  if (!planType) return Swal.fire({ icon: 'error', title: 'ข้อผิดพลาด', text: 'กรุณาเลือกประเภทแผน' })
  ...
  try {
    if (isEditMode.value) {
      await axios.post(`${API}/Edtdataplans`, basePayload)
      await fetchPlans()
...
function confirmRemoveStep(i) {
  Swal.fire({ title: 'ยืนยันการลบ', text: 'ลบขั้นตอนนี้?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6', confirmButtonText: 'ลบ', cancelButtonText: 'ยกเลิก' })
    .then(r => {
      if (r.isConfirmed) {
...
function openEditStepDialog(step, i) {
  showEditStepDialog.value = true
  Object.assign(currentEditingStep, { id: step.id, name: step.name, dates: [step.startDate, step.endDate], index: i })
}
...
async function saveEditedStep() {
  if (!currentEditingStep.name || currentEditingStep.dates?.length !== 2) {
  ...
  try {
    ...
    allPlans.value.forEach(p => {
      const s = p.steps?.find(x => x.id === currentEditingStep.id)
      if (s) { s.name = currentEditingStep.name; s.startDate = currentEditingStep.dates[0]; s.endDate = currentEditingStep.dates[1] }
    })
    const s2 = currentPlan.steps?.find(x => x.id === currentEditingStep.id)
    if (s2) { s2.name = currentEditingStep.name; s2.startDate = currentEditingStep.dates[0]; s2.endDate = currentEditingStep.dates[1] }
...
const isAddStepFormValid = computed(() => !!addStepForm.name && addStepForm.dates?.length === 2 && addStepForm.dates[0] && addStepForm.dates[1])
...
async function saveNewStepForPlan() {
  ...
  try {
    ...
    if (showPlanDialog.value && currentPlan?.id === addStepForm.planId) addToPlanState(currentPlan)
...
async function onTaskTypeChange() {
  ...
  subTasks.value = []
  if (!t) return
  if (t === 'งานหลัก') await fetchPositionMainWorks()
  else if (t === 'งานตำแหน่งอื่น') await fetchAllMainWorks()
}
...
async function openAddTaskDialog(stepData, owners) {
  try {
    ...
    console.error('getMainWorks error:', e)
  }
...
async function addTaskToStepFromMain() {
  ...
  try { 
    ...
    const invalidTime = startNorm && endNorm && endNorm < startNorm
    if (invalidTime) {
      return Swal.fire({ icon: 'error', title: 'ข้อผิดพลาด', text: 'กรุณาตรวจสอบเวลาเริ่มต้น-สิ้นสุด' })
    }
    ...
    const payload = {
      ...
      startTime: toDateTimeStr(startNorm),
      endTime: toDateTimeStr(endNorm),
      status: 'เสร็จสิ้น',
      staff_id: session.staffId,
      fac_id: session.facId,
      ...
      por_detel: newTaskInStep.porForm?.label ?? null,
    }
    ...
    if (step) {
      ...
      step.tasks.push({
        ...
        supervisor: newTaskInStep.supervisor ? { id: newTaskInStep.supervisor.id, name: newTaskInStep.supervisor.name } : null,
...
async function saveEditedTask() {
  ...
  const payload = {
    ...
    responsible: (currentEditingTask.responsible || []).map(o => ({ id: o.id })),
    supervisor_id: currentEditingTask.supervisor?.id ?? null, // ✅
    ...
    if (pIdx !== -1) {
      ...
      if (sIdx !== -1) {
        allPlans.value[pIdx].steps[sIdx].tasks[currentEditingTask.taskIndex] = {
          ...
          supervisor_id: payload.supervisor_id,
          supervisor: currentEditingTask.supervisor ? { id: currentEditingTask.supervisor.id, name: currentEditingTask.supervisor.name } : null,
...
async function confirmRemoveTaskInTable(step, taskIndex) {
  ...
  try {
    await axios.post(`${API}/Deldatatask`, { data: { id: task.id } })
...
async function confirmRemoveStepById(planId, stepId) {
  ...
  if (!r.isConfirmed) return
  try {
    await axios.post(`${API}/Deldatastep`, { data: { id: stepId } })
    const plan = allPlans.value.find(p => p.id === planId)
    if (plan) plan.steps = plan.steps.filter(s => s.id !== stepId)
    Swal.fire({ icon: 'success', title: 'สำเร็จ', text: 'ลบขั้นตอนเรียบร้อยแล้ว', timer: 1200, showConfirmButton: false })
  } catch (e) {
    console.error(e)
...
const taskOwnerSuggestions = ref([])
const supervisorSuggestions = ref([])
let taskOwnerTimer = null
async function searchTaskOwners(e) {
  const q = (e.query || '').trim()
  if (taskOwnerTimer) clearTimeout(taskOwnerTimer)
  if (!q || q.length < 3) { taskOwnerSuggestions.value = []; return }

  taskOwnerTimer = setTimeout(async () => {
    try {
      const res = await axios.get(`${API}/searchDataStaff`, { params: { staffid: q } })
      const arr = Array.isArray(res.data) ? res.data : (res.data?.data || [])
      taskOwnerSuggestions.value = arr.map(mapStaffToOption)
    } catch {
      taskOwnerSuggestions.value = []
    }
  }, 250)
}

let supervisorTimer = null
async function searchSupervisors(e) {
  const q = (e.query || '').trim()
  if (supervisorTimer) clearTimeout(supervisorTimer)
  if (!q || q.length < 3) { supervisorSuggestions.value = []; return }

  supervisorTimer = setTimeout(async () => {
    try {
      const res = await axios.get(`${API}/searchDataStaff`, { params: { staffid: q } })
      const arr = Array.isArray(res.data) ? res.data : (res.data?.data || [])
      supervisorSuggestions.value = arr.map(mapStaffToOption)
    } catch {
      supervisorSuggestions.value = []
    }
  }, 250)
}
...
async function onEditEvaluChange() {
  ...
  try {
    ...
    porFormSuggestions.value = porFormsAll.value
    if (currentEditingTask.porFormCode) {
      const pf = porFormsAll.value.find(it => String(it.value) === String(currentEditingTask.porFormCode))
